...
----* CSD_TRANSFORAMTIONS *----
CREATE TABLE IF NOT EXISTS `CSD_TRANSFORAMTIONS`(
...
ALTER TABLE
    `CSD_TRANSFORAMTIONS` ADD INDEX `csd_transforamtions_source_id_index`(`SOURCE_ID`);

ALTER TABLE
    `CSD_TRANSFORAMTIONS` ADD INDEX `csd_transforamtions_transformation_rule_index`(`TRANSFORMATION_RULE`);
...
ALTER TABLE
    `CSD_TRANSFORAMTIONS` ADD CONSTRAINT `csd_transforamtions_source_id_foreign` FOREIGN KEY(`SOURCE_ID`) REFERENCES `CSD_SOURCES`(`SOURCE_ID`);
    
ALTER TABLE
    `CSD_TRANSFORAMTIONS` ADD CONSTRAINT `csd_transforamtions_target_source_id_foreign` FOREIGN KEY(`TARGET_SOURCE_ID`) REFERENCES `CSD_SOURCES`(`SOURCE_ID`);